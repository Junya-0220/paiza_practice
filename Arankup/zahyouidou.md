...
移動ごとの向きの変化を管理します。移動と向きの関係は規則性があるのでそれらを実装します。
移動は繰り返し行われるので、関数にすると良いです。
移動方向は全部で 4 種類、しかし移動回数は定まっていないので配列の要素の個数はいくつ用意すれば良いのか？
 → カウント変数 d などを用意して、各方向を 配列名[d % 4] とすることで、d の値を変更するだけで任意の方角が得られます。
direct[d%4] = {'N','E','S','W'}; とすれば、右への移動は d++; 左への移動は d+=3; で実装できます。
 左への移動を d--; としてしまうと、 d の値が負になってしまい、d%4 の演算がうまくいかなくなってしまうことがあるので d+=3; とします。

リスト directions と、カウント変数 now_direction で移動方向の管理をしています。
入力された方向が L ならば移動方向を 3 増やし、R ならば移動方向を 1 増やし ます。( 1 減らすでも問題ありません)
if 文を用いて判定し座標を移動させ、座標を出力します
